...
public class ValidatorTests {
    ...
    private static void testValidNames() {
        if (Validator.validateUserName("jaredrobbins"))
            testLogger(TEST_ONE, SUCCESSFUL);
        else
            testLogger(TEST_ONE, FAILED);

        if (Validator.validateUserName("jarobb12345"))
    ...
    private static void testInvalidNames() {
        if (!Validator.validateUserName("j@r3drobb!ns"))
            testLogger(TEST_THREE, SUCCESSFUL);
        else
            testLogger(TEST_THREE, FAILED);

        if (!Validator.validateUserName("jaredrobbins12#456"))
            testLogger(TEST_FOUR, SUCCESSFUL);
        else
            testLogger(TEST_FOUR, FAILED);

        if (!Validator.validateUserName(""))
            testLogger(TEST_FIVE, SUCCESSFUL);
        else
            testLogger(TEST_FIVE, FAILED);

        if (!Validator.validateUserName(null))
            testLogger(TEST_SIX, SUCCESSFUL);
        else
            testLogger(TEST_SIX, FAILED);
    }
